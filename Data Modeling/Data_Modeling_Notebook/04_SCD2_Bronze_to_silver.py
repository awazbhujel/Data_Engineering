...
if not insert_timestamp_param or insert_timestamp_param.strip() == "":
    ...
    bronze_df = spark.table("data_modeling_demo.bronze.scd2")
else:
...
# DBTITLE 1,Write to silver table (SCD Type 2)
# SCD Type 2: Track historical changes
silver_table_name = "data_modeling_demo.silver.scd2"

# Add SCD Type 2 columns to source
from pyspark.sql.functions import current_timestamp, lit

scd2_source = quality_df.withColumn("effective_from", current_timestamp()) \
                        .withColumn("effective_to", lit(None).cast(TimestampType())) \
                        .withColumn("is_current", lit(True))

# Check if silver table exists
if spark.catalog.tableExists(silver_table_name):
    print(f"Table {silver_table_name} exists. Performing SCD Type 2 merge...")
    
    silver_table = DeltaTable.forName(spark, silver_table_name)
    
    # Step 1: Merge to expire changed records and insert new records
    ...
    # Find customer_ids that were just expired
    expired_customers = spark.table(silver_table_name) \
        .filter("is_current = false") \
        .filter("effective_to >= current_timestamp() - interval 5 seconds") \
        .select("customer_id").distinct()
    
    # Get new versions for these customers
    new_versions = scd2_source.join(
        expired_customers,
        ["customer_id"],
        "inner"
    )
    ...
    # Insert new versions
    new_version_count = new_versions.count()
    if new_version_count > 0:
        new_versions.write.format("delta").mode("append").saveAsTable(silver_table_name)
        print(f"✓ Inserted {new_version_count} new versions for changed records")
    ...
    # Create new silver table with SCD Type 2 structure
    scd2_source.write \
        .format("delta") \
        .mode("overwrite") \
...
current_count = spark.table(silver_table_name).filter("is_current = true").count()
...
silver_df = spark.table("data_modeling_demo.silver.scd2")
...
display(silver_df.orderBy(F.col("customer_id")).limit(10))